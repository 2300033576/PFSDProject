...
from django.http import HttpResponse
from .models import usersinformations
    ...
    if request.method == 'POST':
        ...
        if pass1 == pass2:
            ...
            try:
                ...
                return HttpResponse("<h1>There was a problem saving user information. Please try again.</h1>")
        else:
            return render(request, 'shop/signuptobid.html', {'error': 'Passwords do not match.'})
    else:
        return render(request, 'shop/signuptobid.html')
...
def mybids(request):
    ...
    if count == 0:
        param = {'countda': count, 'objs': objs, 'countobj': objs.count()}
    else:
        prod = product.objects.filter(id=da[0].pid)
        param = {
            'bidding': da,
            'nameprod': prod[0] if prod else "No Product",
            'objs': objs,
            'countda': count,
            'countobj': objs.count(),
        }
